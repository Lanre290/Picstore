    ...
    <div class="fixed top-10 right-0 left-0 flex flex-col pt-10">
        <div class="flex flex-col-reverse md:flex-row justify-between w-full">
            ...
            <div class="flex flex-row mr-5 items-center">
                <div class="text-blue-400 cursor-pointer text-5xl m-2" title="{{session('user')->name}}">
                    <i class="fa fa-user-circle"></i>
                </div>
                <input type="hidden" name="csrf-token" value="{{ csrf_token() }}">
                <button class="w-8 h-8 cursor-pointer ml-2" title="logout" onclick="logout()">
                    <img src="{{asset('img/logout.png')}}" alt="logout" class="w-8 h-8">
                </button>
            </div>
        </div>
